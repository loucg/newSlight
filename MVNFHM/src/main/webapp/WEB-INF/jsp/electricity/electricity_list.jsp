							...
							<c:choose>
								<c:when test="${not empty electricityList}">
									...
									<c:forEach items="${electricityList}" var="electricity" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' <c:if test="${electricity.c_gateway_type_id==5}">disabled="true"</c:if> value="${electricity.id}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>
												<c:if test="${electricity.c_gateway_type_id==4}">
													<c:if test="${electricity.staus==0}">已断电</c:if>
													<c:if test="${electricity.staus==1}">上电中</c:if>
												</c:if>
												<c:if test="${electricity.c_gateway_type_id==5}">无法获取</c:if>
											</td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-danger" onclick="makeAll('<%=make_sure_up_and_down_electricity_for_select_device %>?');
										<c:if test="${electricity.c_gateway_type_id==5}">toclose();
										</c:if>" >
									<%=up_and_down_electricity %>
									</a>
