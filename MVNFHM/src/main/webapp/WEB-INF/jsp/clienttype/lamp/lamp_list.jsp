...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="clienttype/getLampList" method="post" name="Form" id="Form">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=add2%>";
			 diag.URL = '<%=basePath%>clienttype/goLampCreate';
			 diag.Width = 650;
			 diag.Height = 324;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=edit%>";
			 diag.URL = '<%=basePath%>clienttype/goLampEdit?id='+Id;
			 diag.Width = 650;
			 diag.Height = 324;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function fromExcel(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=importt%>";
			 diag.URL = '<%=basePath%>clienttype/goLampUploadExcel';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(Id){
			...
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>clienttype/deleteLamp.do?id='+Id+'&tm='+new Date().getTime();
					$.get(url,function(data){
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>删除失败！这个终端类型正在使用中.</span>",
								buttons: 			
								{
									"button" :
									{
