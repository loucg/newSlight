...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="clienttype/getGatewayList" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center"><%=name_ch %></th>
									<th class="center"><%=name_en %></th>
								    ...
								    <th class="center"><%=gateway_type_comment%></th>
								...
								<c:when test="${not empty gatewayList}">
									...
									<c:forEach items="${gatewayList}" var="var" varStatus="vs">
										<tr>
											...
											<td class="center">${var.name_CH}</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=add2%>";
			 diag.URL = '<%=basePath%>clienttype/goGatewayCreate';
			 diag.Width = 650;
			 diag.Height = 424;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=edit%>";
			 diag.URL = '<%=basePath%>clienttype/goGatewayEdit?id='+Id;
			 diag.Width = 650;
			 diag.Height = 424;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function fromExcel(){
			 ...
			 diag.Drag=true;
			 diag.Title ="<%=importt%>";
			 diag.URL = '<%=basePath%>clienttype/goGatewayUploadExcel';
			 diag.Width = 300;
			 diag.Height = 150;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>clienttype/deleteGateway.do?id='+Id+'&tm='+new Date().getTime();
					$.get(url,function(data){
						if("success" == data.result){
							...
							top.hangge();
							bootbox.dialog({
								message: "<span class='bigger-110'>删除失败！这个网关类型正在使用中.</span>",
								buttons: 			
								{
									"button" :
									{
