			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="strategyGroup/listGroups.do?" method="post" name="Form" id="Form">
...
<%-- 								<th class='center'>“${strategyforpage.name }<%=strategy %>” <%=app_list %></th> --%>
								...
								<c:when test="${not empty strategyGroup}">
									...
									<c:forEach items="${strategyGroup}" var="strategyGroup" varStatus="vs">
										<tr>
											...
											<td class='center'>${strategyGroup.number}</td>  
										</tr>
						...
						<table style="width:100%;">
							<tr>
								...
								<c:if test="${msg != 'view'}">
									<c:if test="${QX.del == 1 }">
									<a class="btn btn-mini btn-danger" onclick="makeAll('<%=make_sure_delete_app_group %>',${pd.strategy_id});" title="<%=multiple_delete_app_group %>" ><%=keck_delete_app_group %></a>
									</c:if>
								</c:if>
								</td>
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '<%=make_sure_delete_app_group %>'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>strategyGroup/removeGroups.do?strategy_id='+strategy_id,
						    	data: {DATA_IDS:str},
								dataType:'json',
								cache: false,
