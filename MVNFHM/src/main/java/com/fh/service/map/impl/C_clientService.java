...
public class C_clientService implements C_clientManager {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<c_client> getALLClient(c_client cc) throws Exception {

		List<c_client> adm = (List<c_client>) dao.findForList("C_clientMapper.queryAllclient", cc);
		c_client c = new c_client();
		for (int i = 0; i < adm.size(); i++) {
			c = adm.get(i);
			String coordinate = c.getCoordinate();
	...
	@Override
	@SuppressWarnings("unchecked")
	public void insertPartMapHeader(c_partmap cp) throws Exception {
		dao.update("C_clientMapper.insertPartMapHeader", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public void insertPartMapDetail(c_partmap cp) throws Exception {
		dao.update("C_clientMapper.insertPartMapDetail", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public List<c_partmap> getPartMapInfo(c_partmap cp) throws Exception {
		return (List<c_partmap>) dao.findForList("C_clientMapper.getPartMapInfo", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public void updatePartMapHeader(c_partmap cp) throws Exception {
		dao.findForList("C_clientMapper.updatePartMapHeader", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public void delPartMapDetail(c_partmap cp) throws Exception {
		dao.findForList("C_clientMapper.delPartMapDetail", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public void delPartMapHeader(c_partmap cp) throws Exception {
		dao.findForList("C_clientMapper.delPartMapHeader", cp);
	}

	@Override
	@SuppressWarnings("unchecked")
	public List<PageData> getCountClientfromPartMap(Page page) throws Exception {
		return (List<PageData>) dao.findForList("C_clientMapper.getCountClientfromPartMaplistPage", page);
	}
	...
	public void updateClientAttr_brightness(c_client cc) throws Exception {
		...
		fhlogService.saveDeviceLog(UserUtils.getUserid(), "调节亮度", ids, null, CMDType.ADJUST_BRIGHTNESS, null);
	...
	public void updateClientAttr_status(c_client cc) throws Exception {
		...
		if (cc.getBrightness() != 0) {
			...
			fhlogService.saveDeviceLog(UserUtils.getUserid(), "关灯", ids, null, CMDType.TURN_OFF, null);
		} else {
			updateClientAttr_statusON(cc);
			fhlogService.saveDeviceLog(UserUtils.getUserid(), "开灯", ids, null, CMDType.TURN_ON, null);
		}
	}
	...
	public void updateClientDraw_status(draw_client dc) throws Exception {
		...
		if (dc.getTakeid() == 1) {
			...
			fhlogService.saveDeviceLog(UserUtils.getUserid(), "开灯", strings, null, CMDType.TURN_ON, null);
		} else if (dc.getTakeid() == 2) {
			// List list=dc.getDrawid();
			dao.update("C_clientMapper.updateClientDraw_statusOFF", list);
			fhlogService.saveDeviceLog(UserUtils.getUserid(), "关灯", strings, null, CMDType.TURN_OFF, null);
		} else {
			// List list=dc.getDrawid();
			System.out.println("****************************************");
			...
			fhlogService.saveDeviceLog(UserUtils.getUserid(), "调节亮度", strings, null, CMDType.ADJUST_BRIGHTNESS, null);
		}

	}

	/**
	 * 根据局部ID和clientId查询灯所在局部图数目
	 * 
	 * @param cp
	 * @return int 局部图数目
	 */
