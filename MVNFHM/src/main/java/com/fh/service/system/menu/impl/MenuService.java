...
public class MenuService implements MenuManager{
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<Menu> listSubMenuByParentId2(String parentId) throws Exception {
		return (List<Menu>) dao.findForList("MenuMapper.listSubMenuByParentId2", parentId);
	}
	...
	/**
	 * 获取所有菜单并填充每个菜单的子菜单列表(权限管理列表)(递归处理)
	 * @param MENU_ID
	 * @return
	 * @throws Exception
	 */
	public List<Menu> listAllMenuQx2(String MENU_ID) throws Exception {
		List<Menu> menuList = this.listSubMenuByParentId2(MENU_ID);
		for(Menu menu : menuList){
			menu.setSubMenu(this.listAllMenuQx(menu.getMENU_ID()));
			menu.setTarget("treeFrame");
		}
