...
//			mv.addObject("roleList", roleList);
	...
	public ModelAndView listAllMenu(Model model,String ROLE_ID)throws Exception{
		...
		try{
			...
			List<Menu> menuList = menuService.listAllMenuQx2("0");	//获取所有菜单
			menuList = this.readMenu(menuList, roleRights);			//根据角色权限处理菜单权限状态(递归处理)
			JSONArray arr = JSONArray.fromObject(menuList);
			String json = arr.toString();
			json = json.replaceAll("MENU_ID", "id").replaceAll("PARENT_ID", "pId").replaceAll("MENU_NAME_EN", "nameEN").replaceAll("MENU_NAME", "name").replaceAll("subMenu", "nodes").replaceAll("hasMenu", "checked");
