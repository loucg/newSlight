...
public class RepairController extends BaseController{
	...
	public ModelAndView listGateway(Page page) throws Exception{
		...
		String faultNo = pd.getString("fault_no");					//检索条件
		if(null != faultNo && !"".equals(faultNo)){
			pd.put("fault_no", faultNo.trim());
		}
		...
		String starttime = pd.getString("Start");					//故障类型
		if(null != starttime && !"".equals(starttime)){
			pd.put("Start", starttime.trim());
		}
		String endtime = pd.getString("End");					//故障类型
		if(null != endtime && !"".equals(endtime)){
			pd.put("End", endtime.trim());
		}
		if((null == starttime || "".equals(starttime))&&(null == endtime || "".equals(endtime)))
		{
	        ...
	        pd.put("Start", firstday.trim());
	        pd.put("End", lastday.trim());
		}
